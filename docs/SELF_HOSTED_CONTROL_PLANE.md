...
`CAPTCHA_SOLVER_ENABLED` (off by default) enables third-party CAPTCHA solving via 2Captcha, CapSolver, Anti-Captcha, or CapMonster (`CAPTCHA_SOLVER_PROVIDER`). Solving only runs against exact origins in `CAPTCHA_SOLVER_ALLOWED_ORIGINS`; anything else returns `403` and does nothing. Only list origins you are authorized to automate. `CAPTCHA_SOLVER_MODE` (`off` | `detect-only` | `auto`) adds a dry-run mode; `CAPTCHA_SOLVER_ALLOW_ANY_ORIGIN` is a dangerous opt-in that bypasses the allowlist (every solve is logged). `CAPTCHA_SOLVER_IMAGE_ENABLED` is a separate gate for the image/puzzle pipeline (page pixels/audio are exfiltrated to the provider). `CAPTCHA_SOLVER_ALLOW_PROVIDER_EXTENSION` loads a provider's own browser extension as an escape hatch that operates outside the origin gate. See [CAPTCHA_SOLVER.md](./CAPTCHA_SOLVER.md) for configuration, API, and the authorization model.
